...
export default function PathSelector({ paths, selectedId, activeId, onPreview }: PathSelectorProps) {
  return (
      ...
      {paths.map((path) => {
        ...
        return (
          <button
            ...
            className={`text-left rounded-lg p-3 border cursor-pointer transition-colors ${
              isSelected
          ...
          >
            <div className="flex items-start justify-between gap-2 mb-1">
              <span className="flex items-center gap-2 text-xs text-ink font-medium leading-snug">
                {path.title}
                {isActive && (
                  <span className="mono text-2xs text-accent border border-[var(--accent-line)] rounded-full px-1.5 py-px">
                    active
                  </span>
                )}
              </span>
